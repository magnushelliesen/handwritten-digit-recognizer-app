...
import numpy as np
from matrix_mapper.matrix_mapper import matrix_mapper  # pyright: ignore
from functions import center_input
import matplotlib.pyplot as plt

from numpy.typing import NDArray
from streamlit_drawable_canvas import CanvasResult  # pyright: ignore
from neural_network import NeuralNetwork


def recognize_on_click(drawing: CanvasResult, nn: NeuralNetwork) -> None:
    drawing_array: NDArray[np.float128]
    drawing_array = np.mean(
        np.array(drawing.image_data)[:, :, :3], axis=2  # pyright: ignore
    )
...
def print_steps(nn: NeuralNetwork) -> None:
    with st.container(border=True):
        ...
        fig, ax = plt.subplots(figsize=(4, 4), frameon=False)  # pyright: ignore
        plt.imshow(  # pyright: ignore
            st.session_state.drawing_array_resized, cmap="plasma"
        )
        plt.xticks([])  # pyright: ignore
        plt.yticks([])  # pyright: ignore
        st.pyplot(fig)
        ...
        fig, ax = plt.subplots(  # pyright: ignore
            nrows=1,
            ncols=nn.n_hidden,
            figsize=(nn.n_hidden * 2, 2),
            frameon=False,
        )

        for i, activation in enumerate(nn.last_activations[:-1]):  # pyright: ignore
        ...
        fig, ax = plt.subplots(figsize=(4, 2), frameon=False)  # pyright: ignore
        ax.bar(
        ...
        plt.yticks(ticks=np.linspace(0, 1, 6))  # pyright: ignore
        st.pyplot(fig)

...
with st.container(width=400):
    ...
    with st.container(border=True):
        st.markdown(
            ...
            It's hosted on Google Cloud Run and uses a pre-trained neural network, which is stored on Google Cloud Storage.
    ...
    st.subheader("About the neural network")
    with st.container(border=True):
        st.markdown(
            f"""
            The neural network has {nn.n_hidden: ,.0f} hidden layers, with {nn.dim_hidden: ,.0f} nodes each.
            It has been trained showing it {nn.training: ,.0f} random draws of digits from the

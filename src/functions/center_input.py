...
def center_input(input: NDArray[np.float128]) -> NDArray[np.float128]:
    ...
    if non_zero_coords.size == 0:
        return np.zeros_like(
            input
        )  # Return a blank array if no non-white elements exist
    ...
    cropped = input[
        max(0, start_row) : min(end_row, input.shape[0]),
    ...
    centered[
        offset_row : offset_row + cropped.shape[0],
        offset_col : offset_col + cropped.shape[1],
    ] = cropped
